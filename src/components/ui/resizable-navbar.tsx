  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-black/90 border border-[#30D411]/50",
        className,
      )}
    >
      {children}
    ...
    >
      {items.map((item, idx) => (
        <Link
          ...
          onClick={onItemClick}
          className={cn(
            "relative px-4 py-2 text-neutral-600 dark:text-neutral-300 transition-colors hover:text-black",
            className
          )}
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-[#30D411] dark:bg-[#30D411]"
            />
          )}
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-md bg-[#30D411] text-black text-sm font-bold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center";
